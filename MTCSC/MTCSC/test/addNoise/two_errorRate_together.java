...
public class two_errorRate_together {
    public static void main(String[] args) {
        ...
        for(int i=0; i<10; i++){
            ...
            //     if(isNormalize){
            //         writefilename = "two/errorRate/together/nor/"+String.valueOf(drate)+"_"+String.valueOf(seed)+"_nor.csv";
            ...
            //     }
            //     else{
            //         writefilename = "two/errorRate/together/ori/"+String.valueOf(drate)+"_"+String.valueOf(seed)+".csv";
            //         TimeSeries2 cleanSeries = assist.readData2_size(inputFileName, ",", size);
            //         TimeSeries2 dirtySeries = assist.addNoiseGPS_maxmin_together(cleanSeries, drate, seed);
            //         assist.saveDataFromNoise(writefilename, dirtySeries);
            //     }
            ...
            for (int j = 0; j < expTime; j++,seed++) {
                if(isNormalize){
                    writefilename = "two_noLabel/errorRate/together/nor/"+String.valueOf(drate)+"_"+String.valueOf(seed)+"_nor.csv";
                    TimeSeries2 cleanSeries = assist.readData2_size(inputFileName, ",", size);
                    ...
                    writefilename = "two_noLabel/errorRate/together/ori/"+String.valueOf(drate)+"_"+String.valueOf(seed)+".csv";
                    TimeSeries2 cleanSeries = assist.readData2_size(inputFileName, ",", size);
                    TimeSeries2 dirtySeries = assist.addNoise2_maxmin_together(cleanSeries, drate, seed);
                    assist.saveDataFromNoise_noLabel(writefilename, dirtySeries);
                }
