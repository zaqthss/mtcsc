...
public class ECG_errorRate_together {
    public static void main(String[] args) {
        ...
        String inputFileName = "ECG/ECG_all.csv";
        int n = 32;
        ...
        for(int i=0; i<10; i++){
            ...
            //         TimeSeriesN cleanSeries = assist.readDataN_ECG(inputFileName, ",", n, size);
            ...
            for(int j=0; j<expTime; j++, seed++){
                if(isNormalize){
                    writefilename = "ECG_noLabel/errorRate/together/nor/"+String.valueOf(drate)+"_"+String.valueOf(seed)+"_nor.csv";
                    TimeSeriesN cleanSeries = assist.readDataN_ECG(inputFileName, ",", n, size);
                    TimeSeriesN dirtySeries = assist.addNoiseN_maxmin_together(cleanSeries, drate, seed, n);
                    assist.normalizeN(dirtySeries, n);
                    ...
                    writefilename = "ECG_noLabel/errorRate/together/ori/"+String.valueOf(drate)+"_"+String.valueOf(seed)+".csv";
                    TimeSeriesN cleanSeries = assist.readDataN_ECG(inputFileName, ",", n, size);
                    TimeSeriesN dirtySeries = assist.addNoiseN_maxmin_together(cleanSeries, drate, seed, n);
                    assist.saveDataFromNoise_noLabel(writefilename, dirtySeries, n);
                }  
